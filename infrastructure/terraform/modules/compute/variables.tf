...
# -----------------------------------------------------------------------------
# Phase 8D Enhanced Lambda Configuration
# -----------------------------------------------------------------------------
variable "search_memory_size" {
  description = "Memory size for search engine Lambda function in MB"
  type        = number
  default     = 512

  validation {
    condition     = var.search_memory_size >= 128 && var.search_memory_size <= 10240
    error_message = "Lambda memory size must be between 128MB and 10240MB."
  }
}

variable "analytics_timeout" {
  description = "Timeout for analytics engine Lambda function in seconds"
  type        = number
  default     = 300

  validation {
    condition     = var.analytics_timeout >= 30 && var.analytics_timeout <= 900
    error_message = "Lambda timeout must be between 30 and 900 seconds."
  }
}

variable "analytics_memory_size" {
  description = "Memory size for analytics engine Lambda function in MB"
  type        = number
  default     = 1024

  validation {
    condition     = var.analytics_memory_size >= 128 && var.analytics_memory_size <= 10240
    error_message = "Lambda memory size must be between 128MB and 10240MB."
  }
}

variable "redis_cluster_endpoint" {
  description = "Redis cluster endpoint for caching integration"
  type        = string
  default     = ""
}
